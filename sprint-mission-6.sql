...
SELECT * 
FROM order_details
WHERE pizza_id IN (SELECT pizza_id FROM pizzas WHERE price > 20);
...
FROM pizzas p
JOIN order_details d ON d.pizza_id = p.id
GROUP BY p.id
...
/*
    3. `order`의 `id`가 78에 해당하는 주문 내역들을 조회합니다. 주문 내역에서 각각 주문한 피자의 이름을 `pizza_name`, 피자의 크기를 `pizza_size`, 피자 가격을 `pizza_price`, 수량을 `quantity`, 각 주문 내역의 총 금액을 `total_amount` 라는 이름으로 조회해 주세요.
        ...
        ```
*/
SELECT t.name AS pizza_name, p.size AS pizza_size, p.price AS pizza_price, SUM(d.quantity) AS quantity, SUM(p.price * d.quantity) AS total_amount
FROM order_details d JOIN pizzas p ON d.pizza_id = p.id
JOIN pizza_types t ON p.type_id = t.id
WHERE d.order_id = 78
GROUP BY t.name, p.size, p.price;
...
/*    
    4. `order_details`와 `pizzas` 테이블을 JOIN해서 피자 크기별(S, M, L) 총 수익을 계산하고, 크기별 수익을 출력하세요.
        ...
        ```
*/
SELECT p.size, SUM(p.price * d.quantity) AS total_revenue
FROM pizzas p
JOIN order_details d ON d.pizza_id = p.id
GROUP BY p.size;
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
SELECT t.name, SUM(p.price * d.quantity) AS total_revenue
FROM order_details d
JOIN pizzas p ON d.pizza_id = p.id
JOIN pizza_types t ON p.type_id = t.id
GROUP BY t.name
